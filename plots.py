...
def all_models_lexical_stats_plot(
    ...
    question_stats_file = "results/questions/questions_lexical_distances_stats.json"
    try:
...
def all_models_word_count_plot(
    ...
    data_file = f"results/questions/questions_lexical_distances_stats.json"
    stats = dd.load.Json(data_file)
    ...
    plt.plot(variation_names, word_counts, marker='o', linewidth=2, 
            markersize=8, color='purple', label='Original Variation')
...
def directions_plot(
    ...
    for i, variation in enumerate(variations):
        ...
        # Get distance (length of arrow) using euclidean distance
        t_distance = stats[variation][distance_type]["mean"]
...
def stats_boxplot(
    ...
    # Extract variation names (excluding 'question')
    variation_names = variations[1:]  # Exclude 'question'
    ...
    for variation in variation_names:
        stats = stats_data[variation][variable_type]
        # Create pseudo-boxplot data from statistics
...
def questions_lexical_stats_plot(
    variations: list,
    show: bool = False,
    comparison: str = "question",
    data_file: str = "results/questions/questions_lexical_distances_stats.json",
    save_file: str = "results/questions/plots/questions_lexical_stats_plot.png") -> bool:
    """
